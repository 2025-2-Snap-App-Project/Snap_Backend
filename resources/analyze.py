...
def gemini_summary(ocr_text: str):
    ...
    **주의사항(모두 필수로 지켜야 함):**
    ...
    3. summary의 각 항목은 다음 조건을 모두 포함해야 합니다:
    - 반드시 번호로 시작: "1.", "2.", "3." 형태
    - "해당 원재료 또는 첨가물이 무엇인지에 대한 설명" **반드시 포함**
    - "섭취 시 주의해야 할 점 또는 알레르기 위험" **반드시 포함**
    - 즉, 각 항목에는 **식품첨가물 설명 + 섭취 시 주의사항**이 한 문장 안에 모두 있어야 함
    4. summary 문장은 다음 구조를 따르세요: "{'번호'}. {'원재료명'}은(는) {'기능·역할 설명'}에 사용되며, {'섭취 시 주의사항 또는 알레르기 정보'}."
    - 공백 포함 **100~125 byte 길이**로 작성하세요.
    - 문장 길이가 너무 짧거나 단순 요약이 되지 않도록 하세요.
    5. summary 각 문장은 불필요한 강조(markdown bold)를 포함하지 말고 일반 텍스트로만 작성하세요. 강조 효과가 필요한 경우에도 마크다운 형식은 사용하지 않습니다.
    6. 불필요한 문장은 제거하세요.
